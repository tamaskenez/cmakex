...
void save_json_output_archive(string_par path, const T& x)
{
    ...
    if (!f.good())
        throwf("Can't open %s for writing.", path_for_log(path).c_str());
    string what;
    try {
    ...
    throwf("Can't write %s, reason: %s", path_for_log(path).c_str(), what.c_str());
}
...
void load_json_input_archive(string_par path, T& out)
{
    ...
    if (!f.good())
        throwf("Can't open existing file %s for reading.", path_for_log(path).c_str());
    string what;
    try {
        ...
        throwf("Can't read %s, reason: %s", path_for_log(path).c_str(), e.what());
    } catch (...) {
